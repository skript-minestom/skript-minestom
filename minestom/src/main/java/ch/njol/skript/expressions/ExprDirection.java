...
public class ExprDirection extends SimpleExpression<Direction> {
	...
	public boolean init(final Expression<?>[] exprs, final int matchedPattern, final Kleenean isDelayed, final ParseResult parseResult) {
		...
		switch (matchedPattern) {
			case 0:
				direction = new Vec(byMark[parseResult.mark].x(), byMark[parseResult.mark].y(), byMark[parseResult.mark].z());
				if (exprs[1] != null) {
	...
	protected Direction[] get(final Event e) {
		...
		if (direction != null) {
			...
			while (d != null) {
				...
				v.add(d.direction.mul(n2.doubleValue()));
				d = d.next;
			}
			assert v != null;
			...
			if (o instanceof Block) {
				...
				if (f == null || horizontal && (f == BlockFace.UP || f == BlockFace.DOWN))
					return new Direction[] {Direction.ZERO};
				return new Direction[] {new Direction(f, ln)};
			} else {
				final Pos l = ((Entity) o).getPosition();
				if (!horizontal) {
					if (!facing) {
						final Vec v = l.direction().normalize().mul(ln);
						assert v != null;
						return new Direction[] {new Direction(v)};
					}
					final double pitch = Direction.pitchToRadians(l.pitch());
				...
				double yaw = Direction.yawToRadians(l.yaw());
